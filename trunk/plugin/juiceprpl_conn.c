...
#include "juiceprpl_conn.h"

static void fb_attempt_connection(JuiceConnection *);
...
static void fb_update_cookies(JuiceAccount *fba, const gchar *headers)
{
...
static void fb_connection_process_data(JuiceConnection *fbconn)
{
	ssize_t len;
	gchar *tmp;
	...
	if (fbconn->callback != NULL) {
		purple_debug_info("juice", "executing callback for %s\n", fbconn->url);
...
static void fb_fatal_connection_cb(JuiceConnection *fbconn)
{
	PurpleConnection *pc = fbconn->fba->pc;

	purple_debug_error("juice", "fatal connection error\n");
		...
		const gchar *error_message)
{
	...
	if (error_message)
	{
		purple_debug_error("juice", "post_or_get_connect failure to %s\n", fbconn->url);
		purple_debug_error("juice", "post_or_get_connect_cb %s\n",
				error_message);
		fb_fatal_connection_cb(fbconn);
		return;
	}
		...
		PurpleSslConnection *ssl, PurpleInputCondition cond)
{
	...
	purple_debug_info("juice", "post_or_get_ssl_connect_cb\n");
 ...
 */
static gchar *fb_cookies_to_string(JuiceAccount *fba)
{
	GString *str;
		...
		gboolean keepalive)
{
	...
	if (host == NULL)
		host = purple_account_get_string(fba->account, "host", NULL);
	...
	purple_debug_info("juice", "getting url %s\n", url);
	...
	if (method == FB_METHOD_POST)
		purple_debug_info("juice", "sending request data:\n%s\n",
			postdata);
	...
	fbconn = g_new0(JuiceConnection, 1);
	fbconn->fba = fba;
	fbconn->url = real_url;
...
static void fb_attempt_connection(JuiceConnection *fbconn)
{
	JuiceAccount *fba = fbconn->fba;
	...
	if (time(NULL) - fbconn->request_time > 5) {
		...
		purple_debug_error("juice", "could not connect after retries\n");
		fb_fatal_connection_cb(fbconn);
		return;
	}

	purple_debug_info("juice", "making connection attempt\n");
